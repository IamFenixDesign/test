...
import {
  ...
  todayWeekday,
  weekdayLabel,
  isDigitalOrOnlineExclusiveDiscount,
} from './discounts'
...
function qtyStep(unit) {
  // Botones +/- en listado: 0,1 kg
  return unit === 'kg' ? 0.1 : 1
}

function weightFieldStep(unit) {
  // Cantidad y mínimo por kilo se editan en gramos
  return unit === 'kg' ? 0.1 : 1
}

function weightFieldUnitLabel(unit) {
  return unit === 'kg' ? 'g' : 'u.'
}
...
function formWeightForUnit(value, fromUnit, toUnit) {
  const from = fromUnit === 'kg' ? 'kg' : 'unit'
  const to = toUnit === 'kg' ? 'kg' : 'unit'
  if (from === to) {
    return to === 'kg' ? parseDecimal(value) || 0 : normalizeQty(value, 'unit')
  }
  if (from === 'kg' && to === 'unit') {
...
/** Label de descuento de producto válido en sucursal (omite exclusivo digital/online). */
function productDiscountLabel(product) {
  if (!product?.hasDiscount) return ''
  if (product.onlineExclusive) return ''
  const label = String(product.discountLabel || 'Oferta').trim()
  if (!label) return ''
...
function StoreResult({ product, onPick }) {
  ...
  const offer = productDiscountLabel(product)
  return (
    <button className={`store-result ${product.store}`} type="button" onClick={() => onPick(product)}>
      ...
      <span>
        ...
        {offer ? (
          <span className="store-offer-tag">Con oferta</span>
        ) : (
          <span className="store-offer-tag muted">Sin oferta</span>
        )}
      </span>
    </button>
  )
...
function App() {
  ...
  function applyFormStorePrice(store) {
    ...
    setForm((prev) => {
      ...
      return {
        ...
        quantity: formWeightForUnit(prev.quantity, prev.qtyUnit, qtyUnit === 'kg' ? 'kg' : 'unit'),
        minStock: formWeightForUnit(prev.minStock, prev.qtyUnit, qtyUnit === 'kg' ? 'kg' : 'unit'),
        image:
          store === 'coto'
  ...
  function openEditItem(item) {
    ...
    setForm({
      ...
      quantity: qtyUnitOf(item) === 'kg' ? gramsFromKg(item.quantity) : item.quantity,
  ...
  function applyStoreProduct(product) {
    ...
    setForm((prev) => ({
      ...
      qtyUnit,
      quantity: formWeightForUnit(prev.quantity, prev.qtyUnit, qtyUnit),
      minStock: formWeightForUnit(prev.minStock, prev.qtyUnit, qtyUnit),
      ...
      discountCoto: coto ? productDiscountLabel(coto) : prev.discountCoto,
      discountCarrefour: carrefour
        ? productDiscountLabel(carrefour)
        : prev.discountCarrefour,
      discountDia: dia ? productDiscountLabel(dia) : prev.discountDia,
    }))
    setStoreQuery('')
    setStoreResults({ coto: [], carrefour: [], dia: [], errors: {} })
    setStoreError('')
      ...
      setItems((prev) => {
        ...
        const next = prev.map((entry) => {
          ...
          return {
            ...
            discountCoto: coto ? productDiscountLabel(coto) : entry.discountCoto || '',
            discountCarrefour: carrefour
              ? productDiscountLabel(carrefour)
              : entry.discountCarrefour || '',
            discountDia: dia ? productDiscountLabel(dia) : entry.discountDia || '',
            image:
              source === 'coto'
  ...
  function saveItem(event) {
    ...
    const quantity =
      unit === 'kg'
        ? normalizeQty(kgFromGrams(form.quantity), 'kg')
        : normalizeQty(parseDecimal(form.quantity), 'unit')
    const minStock =
      unit === 'kg'
    ...
    const qtyRaw = parseDecimal(form.quantity)
    if (!Number.isFinite(qtyRaw) || qtyRaw < 0) {
      setError('La cantidad no es válida.')
      return
    }
    if (unit === 'kg' && qtyRaw > 0 && qtyRaw < 0.1) {
      setError('La cantidad por kilo debe ser 0,1 g o más.')
      return
    }
      ...
      {cartOpen ? (
        ...
        >
          ...
          {cartLines.length === 0 ? (
            <div className="empty">
              <h3>Carrito vacío</h3>
              <p>No hay productos en stock bajo o sin stock para comprar.</p>
            </div>
          ) : (
            <>
              <section
                ...
                {cartDisplayLines.map((line) => (
                  ...
                  >
                    ...
                    <div className="cart-line-stats">
                      ...
                      <div className={`cart-stat ${line.webDiscount ? 'is-offer' : ''}`}>
                        <em>Oferta</em>
              ...
              (cartQuote.eligible.length > 0 || cartQuote.excluded.length > 0) ? (
                <div className="cart-split">
                  ...
                  <span>
                    <strong>{cartQuote.withWebOffer || 0}</strong> con oferta
                  </span>
                </div>
              ) : cartLines.length > 0 ? (
                <div className="cart-split">
            ...
            <div className="form-grid">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setForm((prev) => ({
                        ...prev,
                        qtyUnit: 'unit',
                        quantity: formWeightForUnit(prev.quantity, prev.qtyUnit, 'unit'),
                        minStock: formWeightForUnit(prev.minStock, prev.qtyUnit, 'unit'),
                      }))
                    }
                  >
                    Por unidad
                  ...
                  <button
                    ...
                    onClick={() =>
                      setForm((prev) => ({
                        ...prev,
                        qtyUnit: 'kg',
                        quantity: formWeightForUnit(prev.quantity, prev.qtyUnit, 'kg'),
                        minStock: formWeightForUnit(prev.minStock, prev.qtyUnit, 'kg'),
                      }))
                    }
                  >
                    Por kilo
              ...
              <label className="field">
                <span>
                  ...
                  <small className="field-unit">{weightFieldUnitLabel(form.qtyUnit)}</small>
                </span>
                <input
                  type="number"
                  min="0"
                  step={weightFieldStep(form.qtyUnit)}
                  inputMode="decimal"
                ...
                />
                {form.qtyUnit === 'kg' ? (
                  <small className="field-hint">Desde 0,1 g (ej: 500 = 500 g).</small>
                ) : null}
              </label>
              <label className="field">
                <span>
